  ...
  document.querySelectorAll('#ucuaInternalNav .tab').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    ['submit','history'].forEach(id=>document.getElementById(id).classList.remove('active'));
...
function setMsg(id,text,type){const e=document.getElementById(id);e.textContent=text||'';e.className='msg'+(text?' show '+type:'')}
  ...
  async function submitForm(){const btn=document.getElementById('submitBtn');setMsg('submitMsg','','');document.getElementById('submitResult').innerHTML='';btn.disabled=true;btn.textContent='Submitting...';try{const payload={name:document.getElementById('name').value,email:document.getElementById('email').value,observationDateTime:document.getElementById('observationDateTime').value,company:document.getElementById('company').value,observationType:document.getElementById('observationType').value,location:getSelectedLocation(),observation:document.getElementById('observation').value,actionTaken:document.getElementById('actionTaken').value,photo:await fileData()};gs('submitObservation',[payload],r=>{setMsg('submitMsg','Observation submitted successfully.','ok');const extra='<div class="privacy">Use My Submissions and email OTP to review this observation later.</div>';document.getElementById('submitResult').innerHTML=`<div class="result"><div class="ref">${esc(r.reference)}</div><div class="meta">${fmt(r.submittedAt)}</div>${extra}</div>`;btn.disabled=false;btn.textContent='Submit Observation'},e=>{setMsg('submitMsg',e.message||String(e),'err');btn.disabled=false;btn.textContent='Submit Observation'})}catch(e){setMsg('submitMsg',e.message||String(e),'err');btn.disabled=false;btn.textContent='Submit Observation'}}

  function renderRecord(r){
    const recognition=(r.recognition||[]);
    const recognitionHtml=recognition.length?`<div class="userRecognition"><div class="userRecognitionTitle">Recognition</div>${recognition.map(x=>{const icon=x.category==='Best UCUA'?'🏆':'★';return `<div class="userRecognitionItem"><span class="recognitionTag">${icon} ${esc(x.category)}</span>${x.notes?`<div class="recognitionNote"><strong>Recognition Note</strong><br>${esc(x.notes)}</div>`:''}</div>`}).join('')}</div>`:'';
    return `<div class="record"><div class="recordtop"><div><h4>${esc(r.reference)} · ${esc(r.observationType)}</h4><div class="meta">${r.observationDateTime?'Observed: '+fmt(r.observationDateTime)+' · ':''}Submitted: ${fmt(r.submittedAt)} · ${esc(r.location)}</div></div></div>${recognitionHtml}<p><strong>Observation</strong><br>${esc(r.observation)}</p>${r.actionTaken?`<p><strong>Immediate action</strong><br>${esc(r.actionTaken)}</p>`:''}${r.hasPhoto?'<div class="detail" style="margin-top:10px">📎 Photo attached</div>':''}</div>`
  }

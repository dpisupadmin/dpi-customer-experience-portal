...
function publicRecord(r){return {reference:r.reference_no,submittedAt:r.submitted_at,observationDateTime:r.observation_datetime,observationType:r.observation_type||'',location:r.location||'',observation:r.observation||'',actionTaken:r.action_taken||'',hasPhoto:!!(r.photo_storage_path||r.photo_url)};}
...
export async function getMySubmissionsBySession(token){const addr=await verifiedEmail(token);const db=getSupabaseAdmin();const {data,error}=await db.from('observations').select('*').eq('deleted',false).eq('is_anonymous',false).ilike('submitter_email',addr).order('submitted_at',{ascending:false});if(error)throw new Error(`Unable to load submissions: ${error.message}`);return (data||[]).map(publicRecord);}

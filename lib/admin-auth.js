...
function portalUrl() {
  return String(process.env.PUBLIC_PORTAL_URL || 'https://dpi-customer-experience-portal.vercel.app').replace(/\/$/, '');
}
...
export async function getAuthorizedAdmin(email) {
...
export async function requestAdminOtp(emailInput) {
  ...
  const { error } = await supabase.auth.signInWithOtp({
    ...
    options: {
      ...
      emailRedirectTo: portalUrl() + '/?otp_context=admin'
    }
  });
...
export async function verifyAdminOtp(emailInput, tokenInput) {
  ...
  return {
    ...
    expiresAt: new Date(Number(data.session.expires_at) * 1000).toISOString(),
    admin: {
      email: admin.email,
...
export async function requireAdminSession(tokenInput) {
 const token=String(tokenInput||'').trim(); if(!token) throw new Error('Administrator session has expired. Please sign in again.');
 const supabase=authClient(); const {data,error}=await supabase.auth.getUser(token); if(error||!data?.user?.email) throw new Error('Administrator session has expired. Please sign in again.');
 const admin=await getAuthorizedAdmin(normalizeEmail(data.user.email)); if(!admin) throw new Error('Administrator access is not authorized.');
 return {id:admin.id,email:admin.email,displayName:admin.display_name,role:admin.role};
}
export async function endAdminSession(tokenInput){ return {ok:true}; }
